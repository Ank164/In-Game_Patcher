...
namespace SkyPlace {
    namespace {
        ...
        template <class T>
        T GetFunction(REX::W32::HMODULE module, const char* name) {
            return module ? reinterpret_cast<T>(REX::W32::GetProcAddress(module, name)) : nullptr;
        }
    }

    bool IsAvailable() {
        const REX::W32::HMODULE module = GetModule();
        return GetFunction<MoveObject_t>(module, "MoveObject") &&
               GetFunction<IsMovingObject_t>(module, "IsMovingObject");
    }
    ...
    bool MoveObject(RE::TESObjectREFR* ref) {
        ...
        const REX::W32::HMODULE module = GetModule();
        const MoveObject_t moveObject = GetFunction<MoveObject_t>(module, "MoveObject");
        const IsMovingObject_t isMovingObject = GetFunction<IsMovingObject_t>(module, "IsMovingObject");
        if (!moveObject || !isMovingObject) {
            return false;
        }

        const RE::ObjectRefHandle handle = ref->GetHandle();
        moveObject(handle);
